...
local Commands = {}; do
	...
	local function trim(tbl, ...)
		for i = 1, select('#', ...) do
			...
			while table.find(tbl, search) do
				table.remove(tbl, table.find(tbl, search))
			end
		end
	end

	function Commands.new(name, aliases, description, fn, ...)
		local self = {
			Name = name,
			Aliases = aliases,
	...
	function Commands:_fuzzyMatch(command)
		if self.List[command] then
			return command, self.List[command]
		end

		for name, data in next, self.List do
			if name:sub(1, #command) == command then
				return name, data
			end

			for i = 1, #data.Aliases do
				local alias = data.Aliases[i]
				if alias:sub(1, #command) == command then
					return alias, data
				end
			end
		end
	...
	function Commands:_staticMatch(command)
		if self.List[command] then
			return command, self.List[command]
		end

		for _, data in next, self.List do
			local alias = table.find(data.Aliases, command)
			if alias then
				return data.Aliases[alias], data
			end
		end
	end

	function Commands:Match(command, matcher)
		command = command:lower()

		if matcher == commandEnums.FUZZY_MATCHER then
			return self:_fuzzyMatch(command)
		end

		return self:_staticMatch(command)
	end

	function Commands.Execute(text, matcher)
		if text:sub(1, #Commands.Prefix) ~= Commands.Prefix then return end

		local arguments = text:sub(#Commands.Prefix + 1):split(' ')
		local command = table.remove(arguments, 1):lower()

		local data = select(2, Commands:Match(command, matcher))
		if not data then return end

		trim(arguments, '', ' ')

		local numParams, isVararg = debug.info(data.Callback, 'a')
		if numParams == 0 and #arguments > 0 and not isVararg then
			return
		end
		if numParams > 0 and (not table.find(data.Flags, commandEnums.ALLOW_NO_ARGS)) and #arguments == 0 then
			return
		end

		return data.Callback(unpack(arguments))
	end
end

Library.Signal = Signal
Library.Commands = Commands
...
do
	...
	local Window = {}; do
		...
		function Window.new(title)
			...
			minimizeButton.MouseButton1Click:Connect(function()
				minimizeButton.Text = (minimizeButton.Text == 'ï¹˜' and '+') or 'ï¹˜'
	...
	containers.Tab = Tab
	containers.CommandBar = CommandBar
end
